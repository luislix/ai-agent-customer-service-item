...
`src/modules/promotion/` 调用 LLM 生成中文或英文文案，再使用系统 Chrome/Edge 无头截图渲染 1080×1440 图片卡片。每日推广任务只消费前一天已审核的国内选品快照，并在 SQLite 中记录内容审核和渠道交付状态。

- 微信服务号：配置凭证后调用官方素材和草稿接口，首期只创建草稿，不调用公开发布接口。
- 小红书：输出 `cover.png`、`content.png`、`caption.txt` 和 `manifest.json` 发布包；审批通过后可用持久化浏览器自动打开创作后台、上传图片并填写文案，进入 `awaiting_manual_publish`，最终公开发布由运营人工点击。
- 内容任务具有 `(content_date, source_pick_id)` 唯一约束，渠道交付具有 `(content_id, channel)` 唯一约束，重复调度不会重复生成。
...
动态库存、价格、物流等字段后续应由实时 Provider 提供，不能把过期向量结果当作当前事实。接口、字段和验收标准见 [`contracts/商品快照JSONL接口规范.md`](contracts/商品快照JSONL接口规范.md) 和 [`contracts/商品RAG验收测试方案.md`](contracts/商品RAG验收测试方案.md)。
